          ...
          {virtualRows.map((virtualRow) => {
            ...
            return (
              ...
              >
                <td style={{ width: "60px" }}>
                  <span
                    className={`badge badge-xs text-white border-none ${log.status >= 200 && log.status < 400 ? "badge-success" : "badge-error"}`}
                  >
                    {log.status}
                  </span>
                </td>
                <td className="font-bold" style={{ width: "60px" }}>
                  {log.method}
                </td>
                <td
                ...
                <td style={{ width: "70px" }}>
                  {log.protocol && (
                    <span
                      className={`badge badge-xs text-white border-none ${
                        log.protocol === "openai"
                          ? "bg-green-500"
                          : log.protocol === "anthropic"
                            ? "bg-orange-500"
                            : log.protocol === "gemini"
                              ? "bg-blue-500"
                              : "bg-gray-400"
                ...
                <td
                  className="text-gray-600 dark:text-gray-400 truncate text-[10px]"
                ...
                <td className="text-right text-[9px]" style={{ width: "90px" }}>
                  {log.input_tokens != null && (
                    <div>I: {formatCompactNumber(log.input_tokens)}</div>
                  )}
                  {log.output_tokens != null && (
                    <div>O: {formatCompactNumber(log.output_tokens)}</div>
                  )}
                </td>
...
export const ProxyMonitor: React.FC<ProxyMonitorProps> = ({ className }) => {
  ...
  const loadData = async (
    page = 1,
    searchFilter = filter,
    accountEmailFilter = accountFilter,
  ) => {
    if (loading) return;
    setLoading(true);
    ...
    try {
      ...
      if (config && config.proxy) {
        ...
        await invoke("set_proxy_monitor_enabled", {
          enabled: config.proxy.enable_logging,
        });
      }
  ...
  useEffect(() => {
    ...
    const setupListener = async () => {
      ...
      console.debug(
        "[ProxyMonitor] Setting up event listener for proxy://request",
      );
      unlistenFn = await listen<ProxyRequestLog>("proxy://request", (event) => {
        ...
        const alreadyExists = pendingLogsRef.current.some(
        ...
        updateTimeout = setTimeout(async () => {
          ...
          if (currentPending.length > 0) {
            setLogs((prev) => {
              ...
              const uniqueNewLogs = currentPending.filter(
    ...
    if (!body)
      return (
        <span className="text-gray-400 italic">
          {t("monitor.details.payload_empty")}
        </span>
      );
    try {
  ...
  return (
    <div
      className={`flex flex-col bg-white dark:bg-base-100 rounded-xl shadow-sm border border-gray-100 dark:border-base-200 overflow-hidden ${className || "flex-1"}`}
    >
      <div className="p-3 border-b border-gray-100 dark:border-base-200 space-y-3 bg-gray-50/30 dark:bg-base-200/30">
        <div className="flex items-center gap-4">
          ...
          >
            <div
              className={`w-2.5 h-2.5 rounded-full ${isLoggingEnabled ? "bg-white" : "bg-gray-400"}`}
            />
            {isLoggingEnabled
              ? t("monitor.logging_status.active")
              : t("monitor.logging_status.paused")}
          </button>
          ...
          <div className="relative">
            <User
              className="absolute left-2.5 top-2 text-gray-400 z-10"
              size={14}
            />
            <select
          ...
          <div className="hidden lg:flex gap-4 text-[10px] font-bold uppercase">
            ...
            <span className="text-green-500">
              {formatCompactNumber(stats.success_count)} {t("monitor.stats.ok")}
            </span>
            <span className="text-red-500">
              {formatCompactNumber(stats.error_count)} {t("monitor.stats.err")}
            </span>
          </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <span className="text-[10px] font-bold text-gray-400 uppercase">
            {t("monitor.filters.quick_filters")}
          </span>
          {quickFilters.map((q) => (
            <button
              key={q.label}
              onClick={() => setFilter(q.value)}
              className={`px-2 py-0.5 rounded-full text-[10px] border ${filter === q.value ? "bg-blue-500 text-white" : "bg-white dark:bg-base-200 text-gray-500"}`}
            >
              {q.label}
            </button>
          ))}
      ...
      <LogTable
        ...
        onLogClick={async (log: ProxyRequestLog) => {
          ...
          try {
            const detail = await invoke<ProxyRequestLog>(
          ...
          >
            ...
            <div className="px-4 py-3 border-b border-gray-100 dark:border-base-300 flex items-center justify-between bg-gray-50 dark:bg-base-200">
              <div className="flex items-center gap-3">
                ...
                <span
                  className={`badge badge-sm text-white border-none ${selectedLog.status >= 200 && selectedLog.status < 400 ? "badge-success" : "badge-error"}`}
                >
                  {selectedLog.status}
                </span>
                <span className="font-mono font-bold text-gray-900 dark:text-base-content text-sm">
                  {selectedLog.method}
                </span>
                <span className="text-xs text-gray-500 dark:text-gray-400 font-mono truncate max-w-md hidden sm:inline">
                  {selectedLog.url}
                </span>
              </div>
              <button
                onClick={() => setSelectedLog(null)}
                className="btn btn-ghost btn-sm btn-circle text-gray-500 dark:text-gray-400 hover:dark:bg-base-300"
              >
                <X size={18} />
              </button>
            </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-y-5 gap-x-10">
                  <div className="space-y-1.5">
                    <span className="block text-gray-500 dark:text-gray-400 uppercase font-black text-[10px] tracking-widest">
                      {t("monitor.details.time")}
                    </span>
                    <span className="font-mono font-semibold text-gray-900 dark:text-base-content text-xs">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="font-mono text-[11px] flex gap-2">
                      ...
                      <span className="text-green-700 dark:text-green-300 bg-green-100 dark:bg-green-900/40 px-2.5 py-1 rounded-md border border-green-200 dark:border-green-800/50 font-bold">
                        Out:{" "}
                        {formatCompactNumber(selectedLog.output_tokens ?? 0)}
                      </span>
                    </div>
                  </div>
                </div>
                <div className="mt-5 pt-5 border-t border-gray-200 dark:border-base-300">
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                    {selectedLog.protocol && (
                      <div className="space-y-1.5">
                        <span className="block text-gray-500 dark:text-gray-400 uppercase font-black text-[10px] tracking-widest">
                          {t("monitor.details.protocol")}
                        </span>
                        <span
                          className={`inline-block px-2.5 py-1 rounded-md font-mono font-black text-xs uppercase ${
                            selectedLog.protocol === "openai"
                              ...
                              : selectedLog.protocol === "anthropic"
                                ? "bg-orange-100 text-orange-700 dark:bg-orange-900/40 dark:text-orange-400 border border-orange-200 dark:border-orange-800/50"
                                : selectedLog.protocol === "gemini"
                                  ? "bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-400 border border-blue-200 dark:border-blue-800/50"
                    ...
                    <div className="space-y-1.5">
                      <span className="block text-gray-500 dark:text-gray-400 uppercase font-black text-[10px] tracking-widest">
                        {t("monitor.details.model")}
                      </span>
                      <span className="font-mono font-black text-blue-600 dark:text-blue-400 break-all text-sm">
                        {selectedLog.model || "-"}
                      </span>
                    </div>
                    {selectedLog.mapped_model &&
                      selectedLog.model !== selectedLog.mapped_model && (
                        <div className="space-y-1.5">
                          <span className="block text-gray-500 dark:text-gray-400 uppercase font-black text-[10px] tracking-widest">
                            {t("monitor.details.mapped_model")}
                          </span>
                          <span className="font-mono font-black text-green-600 dark:text-green-400 break-all text-sm">
                            {selectedLog.mapped_model}
                          </span>
                        </div>
                      )}
                ...
                {selectedLog.account_email && (
                  <div className="mt-5 pt-5 border-t border-gray-200 dark:border-base-300">
                    <span className="block text-gray-500 dark:text-gray-400 uppercase font-black text-[10px] tracking-widest mb-2">
                      {t("monitor.details.account_used")}
                    </span>
                    <span className="font-mono font-semibold text-gray-900 dark:text-base-content text-xs">
                      {selectedLog.account_email}
                    </span>
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        const success = await copyToClipboard(
                          getCopyPayload(selectedLog.request_body),
                        );
                        if (success) {
                          setCopiedRequestId(selectedLog.id);
                          setTimeout(() => {
                            setCopiedRequestId((current) =>
                              current === selectedLog.id ? null : current,
                            );
                          }, 2000);
                        }
                      }}
                  ...
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        const success = await copyToClipboard(
                          getCopyPayload(selectedLog.response_body),
                        );
                        if (success) {
                          setCopiedRequestId(
                            selectedLog.id
                              ? `${selectedLog.id}-response`
                              : null,
                          );
                          setTimeout(() => {
                            setCopiedRequestId((current) =>
                              current === `${selectedLog.id}-response`
                                ? null
                                : current,
                            );
                          }, 2000);
