...
function AddAccountDialog({ onAdd }: AddAccountDialogProps) {
      ...
      unlisten = await listen("oauth-callback-received", async () => {
        ...
        try {
          ...
          if (
            ...
            errorMsg.includes("环境")
          ) {
            setMessage(t("common.environment_error", { error: errorMsg }));
          } else {
            setMessage(
              `${t("accounts.add.tabs.oauth")} ${t("common.error")}: ${errorMsg}`,
            );
          }
        }
      });
  ...
  const handleOAuthWeb = async () => {
    try {
      ...
      if (!popup) {
        ...
        setMessage(
          t(
            "accounts.add.oauth.popup_blocked",
            "ポップアップがブロックされました",
          ),
        );
        return;
  ...
  const handleCompleteOAuth = () => {
    ...
    handleAction(t("accounts.add.tabs.oauth"), completeOAuthLogin, {
      clearOauthUrl: false,
    });
  };
  ...
  const handleManualSubmit = async () => {
    ...
    setStatus("loading");
    setMessage(
      t("accounts.add.oauth.manual_submitting", "認可コードを送信中..."),
    );

    try {
      await invoke("submit_oauth_code", {
      ...
      setMessage(
        t(
          "accounts.add.oauth.manual_submitted",
          "認可コードを送信しました。バックエンドで処理中です...",
        ),
      );
  ...
  const handleImportCustomDb = async () => {
    try {
      if (!isTauri()) {
        alert(
          t("common.tauri_api_not_loaded") ||
            "Storage import only works in desktop app.",
        );
        return;
      }
        ...
        createPortal(
          ...
          >
            ...
            <div
              ...
              className="fixed top-0 left-0 right-0 h-8 z-[1]"
            />
                        ...
                        <div className="relative group/manual flex gap-2">
                          <div className="relative flex-1">
                            <input
                              ...
                              placeholder={t(
                                "accounts.add.oauth.manual_placeholder",
                              )}
                              value={manualCode}
