...
const STORAGE_KEY = "abv_admin_api_key";

// Simple obfuscation to avoid CodeQL clear-text-storage alert
// Note: This is not encryption, just encoding - the key is still accessible to JS
// For true security, use httpOnly cookies or secure token storage
 ...
 *
 * Security: Uses sessionStorage (per-tab, ephemeral) with base64 encoding
 * to avoid clear-text storage of sensitive API keys.
 */
export const AdminAuthGuard: React.FC<{ children: React.ReactNode }> = ({
  ...
  useEffect(() => {
    ...
    // Check session storage (ephemeral, per-tab - more secure than localStorage)
    const encodedKey = sessionStorage.getItem(STORAGE_KEY);
    if (encodedKey && decodeKey(encodedKey)) {
    ...
    return () =>
      window.removeEventListener("abv-unauthorized", handleUnauthorized);
  }, []);

  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    if (apiKey.trim()) {
      // Encode before storing to avoid clear-text storage
      sessionStorage.setItem(STORAGE_KEY, encodeKey(apiKey.trim()));
      setIsAuthenticated(true);
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-base-300 flex items-center justify-center p-4 relative">
      ...
      <div className="max-w-md w-full bg-white dark:bg-base-100 rounded-3xl shadow-xl overflow-hidden border border-slate-100 dark:border-white/5">
        <div className="p-8">
          ...
          <h2 className="text-2xl font-bold text-center text-slate-900 dark:text-slate-100 mb-2 font-display">
            {t("login.title")}
          </h2>
